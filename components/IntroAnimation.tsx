...
const IntroAnimation: React.FC<IntroAnimationProps> = ({ onComplete, allowExit, mode = 'default' }) => {
  ...
  useEffect(() => {
    ...
    if (mode === 'returning') {
      ...
      else if (stage === 5) advance(800, 6);
      else if (stage === 6) advance(1500, 7);
      else if (stage === 7) onCompleteRef.current();
    }
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none perspective-[1200px]">
      <style>{`
        ...
        .key-path { stroke-dasharray: 1200; stroke-dashoffset: 1200; }
        .animate-draw { animation: drawKey 2s cubic-bezier(0.4, 0, 0.2, 1) forwards; }
        .animate-fill { animation: fillGold 1s ease-out forwards; }
        .animate-insert { animation: insertKey 0.5s cubic-bezier(0.68, -0.55, 0.265, 1.55) forwards; }
        .animate-wait { animation: waitPulse 2s ease-in-out infinite; }
        .animate-turn { transform-origin: 50% 55%; animation: turnKey 0.8s cubic-bezier(0.22, 1, 0.36, 1) forwards; }
        .door-panel { transition: transform 1.5s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.5s ease-out 1s; will-change: transform; }
      `}</style>
        ...
        {mode === 'default' ? (
              ...
              `}>
                 <svg viewBox="0 0 200 200" className="w-full h-full overflow-visible drop-shadow-2xl">
                    ...
                    <path
                        ...
                        d="M 100 40 C 85 40 75 50 75 65 C 75 75 80 82 88 86 L 88 140 L 78 140 L 78 150 L 88 150 L 88 160 L 78 160 L 78 170 L 100 170 L 122 170 L 122 160 L 112 160 L 112 150 L 122 150 L 122 140 L 112 140 L 112 86 C 120 82 125 75 125 65 C 125 50 115 40 100 40 Z M 100 55 C 105 55 108 58 108 63 C 108 68 105 71 100 71 C 95 71 92 68 92 63 C 92 58 95 55 100 55 Z"
           ...
           >
              ...
              <p className="text-[#E6B8A2] text-[10px] md:text-xs font-black uppercase tracking-[0.4em] mb-4 drop-shadow-md">
                {allowExit ? "Sinta-se em casa" : "Preparando tudo..."}
              </p>
